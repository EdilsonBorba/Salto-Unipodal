...
NAME = 'Borba'
TRIAL = '1'
LEG = 'Direita' # 'Direita' ou 'Esquerda'
FILE_OUTPUT = "/Users/borba/Desktop/"

FILE_PATH_ANGLES = "/Users/borba/Desktop/OpenSimData/Kinematics/"
FILE_NAME_ANGLES = "DIREITA3.mot"

FILE_PATH_MARKER = "/Users/borba/Desktop/MarkerData 2/"
FILE_NAME_MARKER = "DIREITA3.trc"
...
def read_sto_to_dataframe(file_path, header_lines):
    """Lê um arquivo .sto e retorna um DataFrame a partir das linhas do arquivo."""
...
def butter_lowpass_filter(data, fc, fs, order):
    """Aplica um filtro passa-baixa de Butterworth."""
    w = fc / (fs / 2)
...
file_path_angles = os.path.join(FILE_PATH_ANGLES, FILE_NAME_ANGLES)
data_angles = read_sto_to_dataframe(file_path_angles, header_lines=11)

file_path_marker = os.path.join(FILE_PATH_MARKER, FILE_NAME_MARKER)
...
if LEG == 'Direita':
    ...
    ankle_flexion = data_angles[:, 11]
    toe_x = data_marker[:, 53]
    toe_y = data_marker[:, 54]
    heel_x = data_marker[:, 59]
    heel_y = data_marker[:, 60]
else:
    hip_flexion = data_angles[:, 14]
    knee_flexion = data_angles[:, 17]
    ankle_flexion = data_angles[:, 18]
